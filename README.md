...
python main.py --all --appendix        # add one slide after the four: every metric for every quarter
```
...
| File | What it is |
|---|---|
| `<company>_board_pack.pptx` | The 4-slide deck: Key metrics, ARR and cash charts, Risks and flags, AI commentary (headline, risks, questions for management). With `--appendix`, one more slide after them: every metric for every quarter |
| `<company>_board_memo.docx` and `.pdf` | The board memo, 1 to 3 pages: title and quarter, AI headline, what changed since the last run (when there is one), key metrics table, flags with values and thresholds, data gaps, AI questions for management grouped by theme, and the same footer as the deck. "AI commentary unavailable" when there's no valid analysis; every computed number is there either way |
...
| `logs/run_<timestamp>.jsonl` | The run log: one line per step per company (below), a new file for every run, from here or the web page. Kept, never overwritten |
...
**Run log.** Every run (a `python main.py ...`, or a Generate or Generate all click on the web page) writes `output/logs/run_<timestamp>.jsonl`, and the printout ends with `Run log saved: output/logs/run_20260918-141503.jsonl`. Each line is one JSON object for one step of one company, written the moment the step ends:

```
{"run": "run_20260918-141503", "source": "command line", "company": "Northwind", "step": "deck", "started_at": "2026-09-18T14:15:05.120", "seconds": 1.412, "result": "ok", "error": null}
```

The steps, in order: `clean`, `metrics and flags`, `what changed`, `metrics workbook`, `AI commentary`, `deck`, `memo`, `manifest`. A step's result is `ok` or `failed` (with the error, and no later step runs); the AI step can also be `skipped`, `reused` (a saved analysis of the same numbers) or `failed` with Claude's validation problems while the company is still built. The last line for each company is `whole company`: its outcome (`built`, `failed`, `skipped`, `stopped`, `timed out`), how long it took from start to finish, and why if it wasn't built. A company with no `whole company` line was cut off (Ctrl+C, or the computer went to sleep). `run_log.py` writes and reads the logs; the web page's **Recent runs** card shows the newest five.
...
python benchmark.py                            # seconds per company run, and how often it reads its workbook (temp folder, no API call)
...
memo.py        the same numbers and analysis as a 1 to 3 page memo: <company>_board_memo.docx and .pdf
...
24. **The run log is a line per step, written as each step ends, in a file per run.** A person asking "why did last night's run take 20 minutes" or "which step broke Fernhollow" needs each step's time and error, which the summary table (one row per company) doesn't keep. JSON Lines (one JSON object per line) rather than one JSON document, because a line is complete the moment it's written: a run that crashes or is stopped still leaves every line up to that moment, where a half-written JSON document can't be read at all. A new file per run, named after its start, so no run can overwrite another's history, and deleting old runs is deleting old files. Companies built side by side share the run's file, with a lock so their lines never mix mid-line. A log that can't be written prints a warning and the run carries on: the board pack matters more than its log. The web page reads the same files, so a run from the command line shows on the page too; a line it can't read is counted, never a traceback.
25. **Every stop says what is wrong, where, and what to do next, in plain words.** The person reading "FAILED" is an analyst the night before a board meeting, not the developer. So a workbook problem reads "Sheet 'KPI Tracker', row 3 (header) is missing columns: pipeline - add a column headed with each name (its cells can stay empty if there's no data)", with no Python name like "ValueError:" in front; a file that isn't an Excel workbook, a missing file, a locked file, a bad key or a lost connection each say what to do, not what the library called it. The one exception is a bug: it reads "unexpected problem, probably a bug in this tool rather than the workbook (KeyError: ...)", keeping the Python name, because that's what whoever fixes it needs, and the traceback is still printed. An API error keeps the API's own words at the end ("Details: ..."), since a low credit balance is only explained there. `tests/test_error_messages.py` pins each message's "what to do next", so an edit can't quietly drop it.
26. **Cleaning and metrics are cached by a hash of their inputs, not passed along by hand.** Each output (the Excel file, the deck, the memo) was written to stand on its own from a workbook path, so one run cleaned the same workbook five times and worked out the same metric table up to eight times. Passing the cleaned table from step to step would have changed every step's signature and the command lines that call them one at a time; a cache in the three functions that do the work (`clean_workbook`, `compute_metrics`, `metric_reasons`) changes none of them. The key is the SHA-256 of the workbook and of its mapping file (for the metrics, a hash of the table's values, labels and types), never a file name, so an edited file or a newly confirmed mapping is always read again; errors are never kept, so a stop is found again with today's words; every answer is copied in and out, so no caller can change another's table; and it keeps at most 32 answers, because the web page runs for days. Result (`python benchmark.py`): 1 read instead of 5, and each company's run 23 to 26% faster. The goldens prove no output changed. The rest of a run is building the files themselves.
27. **Every text color passes WCAG AA contrast on every background it's drawn on, and a test works it out.** A board pack is read on a projector, a laptop in sunlight and a printout, and a partner with weaker eyesight reads it too. WCAG 2's level AA asks for 4.5 : 1 between text and what's behind it (3 : 1 for a checkbox or a chart bar). Checking all of them found two failures: passed-flag green on its light green fill was 4.25 : 1, so `GREEN` went from `1E8449` to `1A7742` (5.03 : 1, the same green, a shade darker); and the template's cover page footer had no color of its own, so it took the master's dark gray on the navy background (about 1.5 : 1). `tests/test_contrast.py` works the ratio out from theme.py's colors, both for a declared list of every pair (`theme.TEXT_PAIRS`) and for pairs read back from what the code really makes: the web page's style sheet, Streamlit's theme, the status colors and every colored piece of text in four built decks. So a new color, or an old one used somewhere new, fails a test before anyone has to squint. The one exception is the opt-in `--draft` watermark: it is see-through on purpose so the numbers under it stay readable, and its words are also in the footer, which passes.
28. **Both charts share one axis style, no label can land on another, and a shrinking quarter never relies on color.** Every $K axis has whole-$K ticks (3 to 6), zero among them, and starts and ends on a labelled gridline, with labels written by the same `format_value` as the tables; every quarter gets the same slot on both charts. The latest value now sits just right of the latest bar or point, the one place nothing else is drawn. Crowded quarter labels are thinned (the latest always stays), a "data missing" too wide for its slot is turned on its side, and a runway title too long for the chart wraps. A net new ARR bar below zero is amber and hatched with an "ARR shrank" legend: amber and navy differ in lightness by 4.8 : 1, so they stay apart in grayscale, which is what survives color blindness. `tests/test_chart_layout.py` renders both charts for the three companies and two made-up extremes (16 quarters in the billions of $K with two blank quarters in a row; 2 quarters of $0-1K) and fails if any label overlaps another, sits on a bar or point, or runs off the picture. The old charts failed it on the extremes.
29. **The full metric table is an opt-in appendix, and it gives up words before it gives up points.** A director who wants every number asks for the metrics workbook, but in the board room the deck is what's open; so `--appendix` (on `main.py` or `build_deck.py`) adds one slide after the four: every metric (19 rows, in metrics.py's order) for every quarter (8 columns). It is off by default because the four slides are the board update; the appendix is reference. It follows the deck's rules: numbers from `format_value`, the 12 pt floor and the same fit check (a table that can't fit stops the build and names the slide), the footer, and the `--draft` watermark when asked for. It didn't fit at first: 20 rows at 12 pt with slide 1's cell padding need 403 pt and the slide has 392, and "n/a (no prior period)" is 121 pt wide in an 84 pt column. Rather than go below 12 pt or split it over two slides, the cells are tighter and the three long reason words become marks ("n/a", "n/m", "∞"), with a key under the table naming each mark and color that is on the slide. "data missing" fits, so the one reason that is a problem to chase keeps its words. Gray and red cells are exactly the metrics workbook's, and a tripped cell is bold as well as red, so it never relies on color. A workbook with more than 8 quarters shows its last 8. `check_deck.py` compares every appendix cell and color with the metrics workbook as Excel displays it, and proves it catches a broken appendix.
...
- 📸 **Slide 4, AI commentary:** "AI-drafted from computed metrics - review before use", the headline, the diagnosis and the questions for management grouped by theme (the 3 risks are on slide 3). Include the footer: after a reset it ends "AI-drafted | not reviewed"; to show "reviewed by ...", run `python approve.py northwind --reviewer NAME`, then `python build_deck.py data/northwind.xlsx`.
...
- [x] **Run log:** `output/logs/run_<timestamp>.jsonl`, one line per step per company with its seconds, result and any error, from the command line and the web page; the portfolio page's Recent runs card reads it (`run_log.py`).
