...
python check_deck.py         # 5 slides, slide 2 numbers match the metrics workbook, no text overflows
...
3. **The deck is always built, even if the AI fails.** Its numbers come from Python, so they're valid either way. Slides 1 and 5 say "AI summary unavailable" and the batch result reads `OK (AI failed)`.
...
- 📸 **Slide 1, Summary:** headline, 3 wins and 3 risks, "6 of 9 flags tripped".
- 📸 **Slide 2, Key metrics:** the table with red, green and gray status cells.
- 📸 **Slide 3, ARR and cash:** the charts, with the Q1 2025 "data missing" gap visible.
- 📸 **Slide 4, Risks and flags:** tripped flags vs thresholds, the combo rule, and the Data gaps line.
- 📸 **Slide 5, Questions for management.**
...
- 📸 **Alderpeak slide 1** (`output/alderpeak_board_pack.pptx`): "0 of 9 flags tripped".
- 📸 **Fernhollow slide 4** (`output/fernhollow_board_pack.pptx`): 7 tripped flags and Rule of 40 "cannot evaluate — missing input".
