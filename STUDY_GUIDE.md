...
| `run_workbooks(args, config)` | Finds the files, checks the API key (unless `--skip-ai`; a missing key exits 1 before any company runs), runs the batch, prints the summary, the AI spend and both warnings, saves the CSV and the batch manifest, returns 0 if no company failed, timed out or was stopped, else 1. | |
...
Run with `python -m pytest -q` (1209 tests, about five minutes). Expected values are **worked out by hand** in comments, not copied from running the code. Tests with `@pytest.mark.parametrize` run the same test on many inputs, each inputs line counting as one test.
