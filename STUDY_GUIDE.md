...
| `reusable_analysis(workbook_path, saved_dir, config)` | The saved `output/<company>_analysis.json` if Claude saw **exactly** today's facts (the whole payload matches) and it still passes the deck's checks; else None. | Used by the web page: generating again for a workbook `main.py` already ran costs nothing. An edited workbook never gets an old analysis. (Moved here from app.py in the final run's Task 2.) |
| `ai_part(workbook_path, actuals, next_budget, config, output_dir, skip_ai, client, reuse_saved)` | The AI step's choice: with `reuse_saved`, a saved analysis of exactly these numbers first (free, recorded as "reused ... (no API call)"); else skip; else `ai_step`. | The web page passes `reuse_saved=True`; the command line never does, so `main.py` behaves as before. |
...
| `run_company(workbook_path, config, skip_ai, client, output_dir, draft, reuse_saved)` | Clean → metrics → flags → gaps → Excel → AI step (unless `--skip-ai`) → deck → memo → manifest, printing a ✓ line for each. Returns a result dict for the summary table. | |
...
| `error_message(error)` | Plain words for an error: `NOT_A_WORKBOOK`, clean.py's own message, or "Something unexpected went wrong ..." (the traceback goes to the Terminal window only). | "No readable workbook shows the plain clean.py message, never a traceback." |
| `load_company(workbook_path, config)` | (everything the deck shows, None), or (None, why it can't be read). Never raises. | The table and the company page both start here. |
| `output_files(workbook_path, output_dir)` | Where the deck, memo PDF, memo Word file and metrics workbook are. | |
| `run_state(workbook_path, output_dir, config_path)` | From the manifest: last run, deck status, and whether the files were built from **today's** workbook and config.yaml (their hashes match). | Not generated → "Not generated yet"; workbook replaced → "Out of date: the workbook has changed since the last run. Generate again." Otherwise `provenance.deck_status`, so the page, main.py and approve.py can't disagree. |
...
| `add_company(file_name, file_bytes, name, config, data_dir, replace)` | Checks the upload in a temporary folder first (a real .xlsx, and clean.py can read it); only then saves it as `data/<name>.xlsx`. An existing company is replaced only when asked. | A workbook with a problem never reaches `data/`, so it can't break the table. |
...
Run with `python -m pytest -q` (609 tests, about a minute and a half). Expected values are **worked out by hand** in comments, not copied from running the code. Tests with `@pytest.mark.parametrize` run the same test on many inputs, each inputs line counting as one test.
