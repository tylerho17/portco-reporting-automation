...
mapping.py      only for a header clean.py doesn't know: proposes a column, a person confirms,
                saved to mappings/<company>.yaml
                              │
                              ▼
metrics.py      compute_metrics()        ─▶ metrics table: 8 quarters × 20 metrics (ratios as decimals)
...
| `metrics_table(actuals)` | Calls every metric function above except `runway_at_next_budget` (a single number, which callers compute separately) and puts the results in one table: a row per quarter, 20 metric columns. `pipeline` (for the combo rule) and `net_burn` (an input, shown so the memo's questions can quote it in dollars) are copied in as they came. | The table `python metrics.py` prints. |
...
**Constants worth knowing:** `DEFAULT_MODEL = "claude-sonnet-5"` (the model comparison's pick), `MAX_ATTEMPTS = 2` (first try + one retry), `MAX_HEADLINE_WORDS = 30`, `MIN_DIAGNOSIS_WORDS = 60` and `MAX_DIAGNOSIS_WORDS = 90`, `MAX_DETAIL_WORDS = 45`, `MAX_DETAIL_SENTENCES = 2`, `RISK_COUNT = 3`, `MIN_QUESTIONS = 8` and `MAX_QUESTIONS = 10`, `MAX_QUESTION_WORDS = 22`, and `THEMES` (the five themes a question may sit under, in the order the deck shows them). Labels (`METRIC_LABELS`, `INPUT_LABELS`) come from `metrics.py`. `SYSTEM_PROMPT` holds Claude's instructions: role, number rules, what to write, framing rules.

| Function / class | What it does, in plain English | Example / why it exists |
|---|---|---|
| `Point` (class) | The shape of one risk: a `title` and a `detail`. | "Retention slipping" / "NRR went from 108.0% to 97.1%…" |
| `Question` (class) | The shape of one question for management: a `theme` and the `question` text. | "Retention decomposition" / "Split NRR (annualized) of 97.1% into GRR (annualized) and expansion…" |
| `BoardSummary` (class) | The shape of the whole answer: a headline, a diagnosis paragraph, 3 risks and 8 to 10 themed questions. There are no wins any more. | Passed to the API so Claude's reply must be this JSON. |
...
| `grouped_questions(questions)` | The questions as `[(theme, [question text, ...]), ...]` in `THEMES` order; a theme with no questions is left out. A theme that isn't on the list still comes out, at the end. | The deck, the memo, the web page and `print_commentary` all show questions by theme through this one function. A hand-edited file is shown as it is, not silently missing a question. |
| `slide_fit_problems(summary, payload_text)` | Problems if Claude's text is too long for its boxes even at 12 pt, by calling `build_deck.ai_text_problems`. | `build_deck` is imported inside the function: it imports analyze.py, so importing each other at load time would fail. |
| `metric_spellings()` / `names_a_metric(text)` / `names_a_value(text)` | Whether a question names a metric the data holds (spellings worked out from `metrics.py`'s labels) and quotes a figure with a unit ("97.1%", "11.0 mo"). | "Why did NRR fall?" names a metric but quotes no value, so it fails: a board question should point at a number. |
| `has_any(text, cues)` | True if the text holds any of the cue phrases (a plain, lower-case substring test). | Used to spot a question that demands a decomposition, and one that interrogates a definition. |
| `nrr_has_moved(payload_text)` / `divergence_problems(summary, payload_text)` | If NRR moved by at least 0.1 of a point over the quarters, the first "Retention decomposition" question must be the gross versus net test, naming both NRR and GRR. | Falling gross retention behind steady net retention means new sales are covering a broken retention engine, so the board should see both figures apart. |
| `question_problems(summary, payload_text)` | Every problem with the questions: 8 to 10 of them, a valid theme each, at most 22 words, a named metric and a quoted value, a demand for a decomposition rather than an explanation, at least one on a definition, plus `divergence_problems`. | The board-quality rules, checked by code instead of left to the prompt. |
| `validate_summary(summary, payload_text)` | Returns a list of problems: not exactly 3 risks, empty text, headline too long, diagnosis outside 60 to 90 words, a risk detail over 45 words or 2 sentences, everything `question_problems` finds, numbers not in the data, a direction word the numbers contradict, and text that doesn't fit the slides. An empty list = pass. | The code-enforced part of "Claude never does math". |
...
| `print_commentary(summary)` | Prints headline, diagnosis, risks, then the questions under their themes, with no model name or stats. | Also used for blind scoring in compare_models.py. |
...
- **The appendix is opt-in** (`--appendix`, Task 20). It follows the same rules: numbers written by `format_value`, the same fit check and 12 pt floor, the footer, and the `--draft` watermark when asked for. 20 metric rows by 8 quarter columns is a lot for one slide, so two things are tighter than slide 1: the cells have smaller margins, and the reason words wider than a quarter column become short marks ("n/a (no prior period)" → "n/a", "n/m ..." → "n/m", "∞ (ARR shrank)" → "∞"). A key under the table explains each mark and color that is on the slide. "data missing" fits, so it keeps its words.
...
| `labelled_gap_lines(gaps)` | The Data gaps lines from gaps already keyed by display label (the way the payload keeps them), one line per set of quarters. "None" when there are no gaps. | `gaps_lines` and `payload_gap_lines` both use it, so slide 3 and the fit check group the gaps the same way. |
| `gaps_lines(gaps)` / `gaps_text(gaps)` | The Data gaps line, grouped by the quarters each metric misses: "Q1 2025 + Q2 2025: ARR growth QoQ, ...". | One bullet per group keeps 20 gaps readable. |
...
| `risk_paragraphs(summary)` | **The AI's 3 risks for slide 3:** a heading saying they are AI-drafted, then each risk's title (bold) and detail. With no validated summary the heading is followed by the gray "AI summary unavailable" line. | The risks used to sit on slide 4. A reader is told the risks are missing rather than left to notice empty space. |
| `gaps_and_risks_paragraphs(gap_lines, summary)` | **Slide 3, right-hand column:** the computed Data gaps first, then the AI-drafted risks under them, in one box. | One box, not two: a company with a blank quarter fills half the column with gaps, and a fixed split would leave the risks too little room while the space above went unused. |
| `risks_slide(slide, deck)` | **Slide 3:** flags on the left; Data gaps and the AI's 3 risks on the right. | Two columns: Fernhollow's text didn't fit in one (LEARNINGS). |
| `commentary_boxes(area)` | **Slide 4's four boxes**, top to bottom: the AI-drafted line, the headline, the diagnosis, the question columns. | `ai_text_problems` measures these same boxes, so the check can't drift from the slide. |
| `headline_paragraph(text, from_ai)` | The headline: navy for Claude's, gray for the placeholder. | |
| `column_heading(text)` | A column's bold navy heading, or the heading of a block inside a column. | |
| `diagnosis_paragraphs(text)` | The diagnosis: one paragraph on what the numbers show and what is likely driving it. | Replaces the old wins and risks lists on slide 4. |
| `theme_paragraphs(theme, questions, first_number)` | One theme: its bold navy heading, then its questions, numbered on from `first_number`. | "Retention decomposition", then "1. ...", "2. ...". |
| `question_paragraphs(groups, start)` | Every theme in `groups` one after the other, with the questions numbered straight through from `start`. | Slide 4 used to number a flat list of 3 questions. Now 8 to 10 questions sit under themes. |
| `split_question_columns(groups)` | Breaks the themes into two columns at the point that splits the text most evenly, counted in characters. A theme is never split across the columns. | Two short questions take less room than one long one, and both columns must fit the same height. |
| `commentary_columns(summary)` | Slide 4's question columns: `grouped_questions` (from analyze.py) puts the questions under their themes, `split_question_columns` makes two columns, and the numbering carries on from the first column into the second. | Numbers run 1 to 10 across both columns, so a question can be referred to by its number. |
| `commentary_slide(slide, deck)` | **Slide 4:** "AI-drafted from computed metrics - review before use", the headline, the diagnosis, then the questions for management by theme in two columns. Or the gray placeholder and note, without the AI-drafted line. | The risks moved to slide 3 (see `risk_paragraphs`). |
...
| `payload_gap_lines(payload_text)` | This company's Data gaps lines, read from the payload text (which keeps the gaps by display label), through `labelled_gap_lines`. | The risks share slide 3's column with the gaps, so the fit check needs this company's real gaps, not a guess at an average one. |
| `ai_boxes(summary, payload_text)` | Every box that holds Claude's words, in reading order: slide 3's right-hand column (gaps and risks), then slide 4's headline, diagnosis and question columns. Each comes with its paragraphs and a piece of advice ("shorten the diagnosis"). | Uses the same paragraph functions the slides draw with, so measuring and drawing can't drift apart. |
| `ai_text_problems(summary, payload_text)` | Problems if the risks, headline, diagnosis or questions don't fit their boxes on slides 3 and 4 even at 12 pt. | analyze.py calls it (through `slide_fit_problems`), so an over-long answer gets the retry. |
...
| `gaps_text(result)` | "none", or "20 metrics/flags (blank: Q1 2025)". | |
...
| `gaps_summary(data)` / `company_row(...)` / `portfolio_rows(config, data_dir, output_dir)` | One row per workbook in `data/`: latest quarter, "6 of 9 flags tripped", "20 metrics/flags (blank: Q1 2025)", last run, status. A workbook clean.py can't read gets "-" and its message. | Flags and gaps are worked out from the workbook now, not copied from the last run, so they're never stale. |
...
| `check_appendix_cell(cell, fill_name, where)` / `check_appendix_cells(slide, table, fills, name)` / `check_appendix_slide(presentation, table, fills, name)` | The 4 slides plus the appendix; its title; header "Metric" and the 8 quarters; the 20 metrics in the Metrics sheet's order; every cell equal to Excel's (as a short mark); gray and red exactly where Excel's cells are, bold only on red; the key has every entry it should and no others. |
...
|---|---|---|
| Larkspur | healthy | no flag trips in any quarter; its answer key checks **all 20 metrics**, not just the 8 flags |
...
**Where you see it:** run `python main.py --all --skip-ai`. The summary row says `Fernhollow  7 of 9, 1 cannot evaluate  21 metrics/flags (blank: Q2 2025)`.
...
5. **Where 21 comes from:** `data_gaps` finds 20 metric columns with a gap (every metric misses Q2 2025; QoQ ones also Q3 2025; YoY ones also Q2 2026). Then a flag that can't be evaluated **because of a missing input** is added: `"flag: Rule of 40": ["Q2 2026"]`: 20 + 1 = **21**. `main.blank_quarters` → Q2 2025. `main.gaps_text` → "21 metrics/flags (blank: Q2 2025)".
