...
- eval/                  the evaluation set: eval/make_eval_data.py writes 12 edge-case companies to eval/data/ (exactly at every threshold, blank first/second-to-last/last quarter, zero revenue, negative budget, 2 quarters of history, a repeated and a missing row that must stop), each with a hand-typed answer key; eval/run_eval.py scores clean, metrics, flags and gaps against them and names every mismatch. eval/score_questions.py scores a generated management-question set (an analyze.py JSON, or a markdown list) against the target set in tests/fixtures/target_questions_northwind.md, on five string and structure checks: does it demand a decomposition rather than an explanation, does it name a metric and a value, is it grouped under a theme, does it duplicate another question, does it cover the target's themes. No model call and no judgement of quality: `--rubric` prints the written 1 to 5 rubric a person applies by reading
- docs/RESEARCH_BOARD_PACKS.md   Researched evidence on what a PE board pack holds and what partners ask, the specification for content decisions; section 6's worked ten-question set is the gold standard, saved to tests/fixtures/target_questions_northwind.md with each question's provenance marker. Sourced fact, inference and gap are kept apart: never write an unsourced convention as an industry standard, and never average two benchmark publishers who disagree
...
- metrics.py             every metric, why a value has no number (reasons), flags, data gaps, and the ONE label set (METRIC_LABELS, INPUT_LABELS). 20 metric columns: "Net burn ($K)" is a raw input carried into the table like pipeline (no flag reads it) so the memo's AI questions can quote it and the workbook shows it
- analyze.py             Claude API → JSON (v6: headline, 60 to 90 word diagnosis, 3 risks, 8 to 10 questions grouped under 5 themes; no wins); validate schema + counts + question rules (each names a metric and its value and asks for a decomposition, reconciliation or distance to breach, except a question about a missing input, which asks who owns it and when; a definition/restatement/missing-input question; the gross vs net divergence test first under retention when NRR moved) + sign-aware number check + slide-fit check (slides 3 and 4) + direction check (a direction word its numbers contradict, a "persistent" trend that isn't), retry once on failure. The words the slides hold are measured per company (build_deck.slide_word_limits: the diagnosis holds about 68 words, a risk detail 54 or, beside a blank quarter's data gaps, 20, a question 20 with 8 or 9 questions but 9 with 10), sent to Claude after the system prompt, and named in a too-long retry message; a rejected answer is kept in run_info's attempt_log. PROMPT_VERSION has a checksum guard in tests/test_analyze.py
...
- build_deck.py          step 4: output/<company>_board_pack.pptx on templates/base.pptx; slides: Key metrics, ARR and cash charts, Risks and flags (its 3 AI risks sit under the data gaps, headed AI-drafted), AI commentary (headline + diagnosis + 8 to 10 questions grouped by theme, under "AI-drafted from computed metrics - review before use"). Re-checks the analysis JSON; "AI summary unavailable" if missing, failed, or too long for slide 4 (ai_text_problems) - a company is never left without a deck. Footer carries the git commit, model and review status ("AI-drafted | not reviewed", or "AI-drafted | reviewed by NAME on DATE" once approve.py records a reviewer). The "DRAFT - NOT REVIEWED" watermark is opt-in: --draft (build_deck.py, main.py) stamps every slide of a deck nobody has approved. --appendix (build_deck.py, main.py) adds one optional slide after the four: every metric for every quarter, with a key
- memo.py                the board memo, 1 to 3 pages, Word + PDF (output/<company>_board_memo.docx/.pdf): headline, what changed since the last run, key metrics, flags, data gaps, questions, the deck's footer. Uses the AI text only if the deck would AND every number in it is one the metrics workbook shows; else "AI commentary unavailable". "Reviewed" only if the approval lists the memo
