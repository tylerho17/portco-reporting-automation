...
- analyze.py             Claude API → JSON (v5: headline, 60 to 90 word diagnosis, 3 risks, 8 to 10 questions grouped under 5 themes; no wins); validate schema + counts + question rules (each names a metric and its value and asks for a decomposition, reconciliation or distance to breach; a definition/restatement/missing-input question; the gross vs net divergence test first under retention when NRR moved) + sign-aware number check + slide-fit check (slides 3 and 4) + direction check (a direction word its numbers contradict, a "persistent" trend that isn't), retry once on failure. PROMPT_VERSION has a checksum guard in tests/test_analyze.py
