...
The look comes from theme.py: navy bars and line, slate text, mid gray axes and gap labels,
Arial (else Helvetica, else DejaVu Sans) at the caption size.
"""
...
def label_latest(axis, values, column, below=None):
    ...
    axis.annotate(format_value(column, value), (position, value), xytext=offset, textcoords="offset points",
                  ha="center", va="top" if below else "bottom", fontsize=FONT_SIZE, color=SLATE_HEX,
...
def bar_panel(axis, quarters, values, column):
    ...
    axis.set_title(METRIC_LABELS[column], loc="left", fontsize=FONT_SIZE, color=SLATE_HEX, fontweight="bold")
...
def cash_chart(quarters, ending_cash, runway_text, size_inches):
    ...
    axis.set_title(f"{INPUT_LABELS['ending_cash']}\n{runway_text}", loc="left", fontsize=FONT_SIZE,
                   color=SLATE_HEX, fontweight="bold")
